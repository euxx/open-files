...
export default class OpenFilesPaneView {
    ...
    let that = this
      ...
      setTimeout(function() {
        ...
        if (addIconToElement) {
          ...
          if (entryPath === '') {
            for (key in iconSubstitutionsFileIcons) {
              if (item.getTitle && item.getTitle() === key) {
        ...
        if (item.getTitle && item.getTitle().split(' ')[1] == 'Preview') {
          fileEntryName.classList.remove('icon-file-text');
        ...
        let instances = 1
        let entryTitle = item.getTitle ? item.getTitle() : ''
	...
	updateTitle(item, siblings=true, useLongTitle=false) {
		var entry;
		let title = item.getTitle ? item.getTitle() : '';
